...
<script>
	$(document).ready(function () {
		...
		$('#setup_menu').show();
		$('#add-account_menu').addClass('selected');
	});
</script>
<body>
        ...
        <div style="padding:20px;overflow:auto;padding-top:4px" id="body">
			...
			<table width="50%" style="width:100% !important">
				<tr>
					...
					<td><input type="text" class="form-control" id="uname" placeholder="Username"></td>
				</tr>
				<tr>
				...
				<tr>
					<td>Account Type :</td>
					<td>
						<select id="type" class="form-control" style="background: url(http://127.0.0.1:49526/media/images/arrow_down.png) 98% 50% / 14px no-repeat transparent;width:20%">
	...
	$('#submit').click(function() {
		...
		if(check){
			$.ajax({
				url: 'check/check.php',
				dataType: 'json',
				data: {accUsername:$('#uname').val()},
				success: function(s){
					console.log(s);
					if(s[0]==true){
						showError('id1');
						$('#uname').parent('td').removeClass('has-error');
					}
					else {
						name = $('#accName').val();
						name = name.split();
										...
										if($('#pass1').val().match(/[A-Z]/g) == null)
											...
											if($('#pass1').val().match(/[a-z]/g) == null)
												...
												if($('#pass1').val().match(/[0-9]/g) == null)
													...
													$.ajax({
														url: 'save/saveA.php',
														data: {uname:$('#uname').val(),name:$('#accName').val(),pass:$('#pass1').val(),status:$('input[type=checkbox]').prop('checked'),account:$('#accID').html(),type:$('#type').val()},
														dataType: 'json',
														success: function(s){
															console.log(s);
															$('#body input,#body select, #body button').prop('disabled',true);
															$('.print_info').fadeIn(100);
														},
														error: function(f){
															$('#error').html(f);
															showError('error');
