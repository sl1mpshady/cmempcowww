			...
			$.ajax({
				...
				success: function(s){
					...
					for(var i = 0; i < s.length; i++) {
						s[i][0] = '<a href="view-return.php?i='+s[i][0]+'">'+s[i][0]+'</a>';
						/*if(s[i][1]=='Sales Order')
							s[i][2]='<a href="view-sales_order.php?i='+s[i][2]+'">'+s[i][2]+'</a>';
						else
							s[i][2]='<a href="view-purchase_order.php?i='+s[i][2]+'">'+s[i][2]+'</a>';*/
						list.fnAddData([
						s[i][0],
						s[i][1],
						s[i][2],
...
<div style="padding:25px" class="hidden-print">
    ...
    <div style="border:1px solid lightgray;border-radius:4px">
    	<div style="padding:6px 10px;background-image: linear-gradient(to bottom, #ffffff 0%, gainsboro 158%);border-bottom: 1px solid lightgray;"><span class="glyphicon glyphicon-list"></span> Returns</div>
				...
				<tr>
					<td width="29%">
						...
						<select class="form-control1" style="margin-left:5px;float:right;height: 28px;width:101px">
							...
							<option value="retID">Return ID</option>
...
$(document).ready(function() {
	$('#print').click(function() {
		...
		$('iframe').attr('src','report/returns.php?type='+$('select').val()+'&value='+$('#search').val()+'&from='+$('#from').val()+'&to='+$('#to').val());
