...
if(isset($_SESSION['MM_Username'])){
	if(!isset($_GET['i']) || isset($_GET['i'])==NULL)
		header("Location: view-customer_ledgers.php");
	else{
		mysql_select_db($database_connSIMS, $connSIMS);
		$query_rsCustomer = "SELECT CONCAT(custFName,' ',custMName,' ',custLName) as name FROM customer WHERE custID='".$_GET['i']."' LIMIT 0,1";
		$rsCustomer = mysql_query($query_rsCustomer, $connSIMS) or die(mysql_error());
		$row_rsCustomer = mysql_fetch_assoc($rsCustomer);
		if(mysql_num_rows($rsCustomer)<=0)
			header("Location: view-customer_ledgers.php");
	}
?>
...
<script>
	$(document).ready(function () {
		...
		function getData(){
			...
			$.ajax({
				url: 'fetch/get-ledger.php',
				dataType: 'json',
				type:'get',
				data: {custID:$('#custID').html(),from:$('#from').val(),to:$('#to').val()},
				success: function(s){
					console.log(s);
					list.fnClearTable();
...
<div style="padding:25px" class="hidden-print">
	<div style="font-size: 26px;color: gray;border-bottom: 1px solid lightgray;margin-bottom:0px;" id="header2">Customer Ledger</div>
	<style>
    ...
    <table width="100%" style="margin:10px 0px">
    	<tr>
            ...
            <td><kbd><span id="custID"><?php echo $_GET['i']; ?></span></kbd> <span id="name"><?php echo $row_rsCustomer['name'];?></span></td>
