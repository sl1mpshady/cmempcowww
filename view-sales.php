...
<script>
	$(document).ready(function () {
		...
		/*$('#list').dataTable({
			...
			"bProcessing": true,
			"bServerSide": true,
			"sAjaxSource": "server_processing/sales-order_server_processing.php?from="+$('#from1').val()+"&to="+$('#to1').val(),
		...
		//$('#tools').append('<button class="btn" id="print"><span class="glyphicon glyphicon-print" style="color:navy;margin-right:3px"></span>Print</button>');
			...
			$.ajax({
				...
				success: function(s){
					...
					for(var i = 0; i < s.length; i++) {
						s[i][0] = '<a href="view-sales_order.php?i='+s[i][0]+'">'+s[i][0]+'</a>';
						list.fnAddData([
						s[i][0],
						s[i][1],
						s[i][2],
...
<div style="padding:25px" class="hidden-print">
    ...
    <div style="border:1px solid lightgray;border-radius:4px">
    	<div style="padding:6px 10px;background-image: linear-gradient(to bottom, #ffffff 0%, gainsboro 158%);border-bottom: 1px solid lightgray;"><span class="glyphicon glyphicon-list"></span> Sales Orders</div>
			...
			<table width="100%" style="margin:0px 0px 5px 0px">
				<tr>
					<td width="25%">
						...
						<select class="form-control1" style="margin-left:5px;float:right;height: 28px;width:87px">
...
$(document).ready(function() {
	$('#print').click(function() {
		...
		$('iframe').attr('src','report/sales_orders.php?type='+$('select').val()+'&value='+$('#search').val()+'&from='+$('#from').val()+'&to='+$('#to').val());
