...
if(isset($_GET['temp_list'])){
	if(isset($_GET['purchase'])){
		if($_GET['temp_list'] == 'DELETE')
			...
			new temp_list($_GET['prodID'],$_GET['prodQty'],$_GET['cost'],'',$_GET['accID'],$_GET['temp_list'],'purchase',$_GET['subProdID'],$_GET['subProdQty']);
	}
		...
		if($_GET['temp_list'] == 'DELETE')
			new temp_list($_GET["prodID"],'','','',$_GET["accID"],$_GET['temp_list']);
		else
			new temp_list($_GET["prodID"],$_GET["prodQty"],$_GET["grossPrice"],$_GET["netPrice"],$_GET["accID"],$_GET['temp_list']);
	}
}
else if(isset($_GET['save']) && isset($_GET['payroll'])){
	$deduction =  new Deduction;
	$deduction->_New($_GET['total'],$_GET['customerCredit'],$_GET['customer'],$_GET['accID']);
	return json_encode($deduction->dedID);
}
	...
	return $query;
		...
		$customer->register($_GET['custID'],$_GET['fname'],$_GET['mname'],$_GET['lname'],$_GET['address'],$_GET['bdate'],$_GET['gender'],$_GET['baddress'],$_GET['mobile'],$_GET['civil_status'],$_GET['photo'],$_GET['elem'],$_GET['hs'],$_GET['coll'],$_GET['voc'],$_GET['others'],$_GET['designation'],$_GET['section'],$_GET['dept'],$_GET['accType'],$_GET['limit'],$_GET['expDate']);
}
...
/*else if(isset($_GET['save'])){
	...
	if(isset($_GET['update'])){
		if(isset($_GET['delete']))
			...
			$product->update($_GET['oldID'],$_GET['prodID'],$_GET['prodName'],$_GET['prodCategory'],$_GET['prodStock'],$_GET['prodPrice'],$_GET['prodStatus'],$_GET['prodReorderQty']);
	}
	else
		$product->register($_GET['prodID'],$_GET['prodName'],$_GET['prodCategory'],$_GET['prodStock'],$_GET['prodPrice'],$_GET['prodStatus'],$_GET['reorderQty'],$_GET['sub_prodID'],$_GET['sub_prodQty']);
	$data = array($product);
	echo json_encode($data);
}
	...
	$po = new Database;
	$po->connect("INSERT INTO purchase VALUES(null,'".$_GET['purcCost']."','".$_GET['qty']."',null,'".$_GET['accID']."',False,'".session_id()."','".$_GET['suppName']."','".$_GET['suppAddress']."','".$_GET['note']."','".$_GET['charges']."','".$_GET['date']."')");
	/*$query = $po->connect("SELECT getCount('".session_id()."',".$_GET['accID'].") as purcID");
	...
	*/
	...
	$poID = $fetch[0];
	$po->__close();
	$query = $po->connect("SELECT purcDateTime FROM purchase WHERE purcID='$poID'");
	$fetch = mysql_fetch_array($query);
	$data[]=array($fetch[0]);
	$po->__close();
	...
	mysql_select_db($db_database);
	 $query=mysql_query("SELECT *, getProdCategory(prodID) as Cat,getProdName(prodID) as prodName FROM po_product_list where purcID='$poID'");
		while($product=mysql_fetch_array($query)){
			 $data[]=array($product["Cat"],$product["prodID"],$product["prodName"],number_format($product["prodQuantity"],0),number_format($product["prodCost"]/$product["prodQuantity"],2),number_format($product["prodCost"],2));
		}
	mysql_free_result($query);
	echo json_encode($data);
}
