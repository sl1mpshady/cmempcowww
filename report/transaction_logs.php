...
if($value!=NULL || $value!=''){
	if($type == 'action'){
		...
		$PHPJasperXML->load_xml_file("transaction_logs_1.jrxml");	
	}
	else{
		$PHPJasperXML->arrayParameter=array("from"=>$from,"to"=>$to,"from1"=>$from1,"to1"=>$to1,"value"=>$value);
		$PHPJasperXML->load_xml_file("transaction_logs_2.jrxml");	
	}	
	
}
else {
	$PHPJasperXML->arrayParameter=array("from"=>$from,"to"=>$to,"from1"=>$from1,"to1"=>$to1);
	$PHPJasperXML->load_xml_file("transaction_logs.jrxml");
}
