...
$(document).ready(function () {
	...
	$('#setup_menu').show();
	$('#transaction-logs_menu').addClass('selected');
...
<div style="padding:25px" class="hidden-print">
	<div style="font-size: 26px;color: gray;border-bottom: 1px solid lightgray;<?php echo (isset($_GET['success']) && $_GET['success']!='' && ($_GET['success']=='add' || $_GET['success'] == 'edit' || $_GET['success'] == 'delete') ? 'margin-bottom:0px' : 'margin-bottom:25px')?>" id="header2">Transaction Logs</div>
    ...
    <div style="border:1px solid lightgray;border-radius:4px">
    	<div style="padding:6px 10px;background-image: linear-gradient(to bottom, #ffffff 0%, gainsboro 158%);border-bottom: 1px solid lightgray;"><span class="glyphicon glyphicon-list"></span> Transactions</div>
        <div style="padding:5px">
			...
			<table width="100%" style="margin:0px 0px 5px 0px">
				<tr>
					<td width="29%">
						<div style="float:left;width:72%" class="input-group">
							<span class="input-group-addon" style="background-image: linear-gradient(to bottom, #ffffff 0%, gainsboro 158%) !important;">
								<span class="glyphicon glyphicon-search"></span> 
							</span>
							<input type="text" class="form-control" placeholder="Search" style="font-weight:normal" aria-controls="list" id="search" disabled>
						</div>
...
$(document).ready(function() {
	$('#print').click(function() {
		...
		$('iframe').attr('src','report/transaction_logs.php?type='+$('select').val()+'&value='+$('#search').val()+'&from='+$('#from').val()+'&to='+$('#to').val());
